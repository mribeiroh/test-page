export default async function handler(req, res) {
  ...
  try {
    // 1. Get the latest runs from GitHub
    ...
    // 2. For each run, try to fetch logs and extract Cypress Cloud URL
    const runs = await Promise.all(
      ghData.workflow_runs.map(async (run) => {
        ...
        try {
          ...
          console.warn(`⚠️ Failed to fetch logs for run ${run.id}:`, err.message);
        }
        ...
        return {
          ...
          cypressUrl,               // Cypress link from logs (if found)
